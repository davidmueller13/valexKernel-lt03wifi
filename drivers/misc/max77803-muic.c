...
/* MAX77803 can use 4 different ADC MODE */
/* get current adc mode */
static u8 max77803_muic_get_adcmode(struct max77803_muic_info *info)
{
...
static u8 max77803_muic_set_adcmode(struct max77803_muic_info *info,
				const u8 val)
{
	const u8 reg = MAX77803_MUIC_REG_CTRL4;
...
/* switch adc mode */
static void max77803_muic_adcmode_switch(struct max77803_muic_info *info,
					const u8 val)
{
	const char *name;
	u8 before, after;
	...
	before = max77803_muic_get_adcmode(info);
	if (before == val) {
		pr_info("%s: no need to change the mode\n", __func__);
		return;
	}

	ret = max77803_muic_set_adcmode(info, val);
	if (ret) {
	...
	after = max77803_muic_get_adcmode(info);
	if (after == val) {
...
/* WA for MUIC RESET */
void _max77803_muic_reg_restore(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(reg_list); i++) {
		value = max77803_restore_last_snapshot(reg_list[i]);
...
static ssize_t max77803_muic_reg_restore_node(struct device *dev,
					       struct device_attribute *attr,
					       char *buf)
{
	_max77803_muic_reg_restore();
...
/* WA for MUIC RESET */
static DEVICE_ATTR(unmasking_muic_int, S_IRUGO, max77803_muic_reg_restore_node,
		NULL);
/* WA for MUIC RESET */
...
static void max77803_muic_detach_smart_dock(struct max77803_muic_info *info)
{
	...
	max77803_muic_adcmode_switch(info, ADCMODE_ONESHOT);
						...
						u8 adc, u8 vbvolt, u8 chgtyp)
{
	...
	case CABLE_TYPE_SMARTDOCK_MUIC:
		if (chgtyp == CHGTYP_DEDICATED_CHGR) {
			...
			max77803_muic_adcmode_switch(info, ADCMODE_ALWAYS_ON);
		} else if (chgtyp == CHGTYP_USB) {
			/* clear CDDelay 500ms */
			max77803_muic_clear_cddelay(info);
...
static void max77803_store_init_reg_status(void)
{
	const enum max77803_muic_reg reg_list[] = {
		/* related with OTG test */
		MAX77803_MUIC_REG_CDETCTRL1,
...
void max77803_muic_reg_restore(struct work_struct *work)
{
	_max77803_muic_reg_restore();
}
/* WA for MUIC RESET */
...
/* WA for MUIC RESET */
	max77803_store_init_reg_status();
/* WA for MUIC RESET */
