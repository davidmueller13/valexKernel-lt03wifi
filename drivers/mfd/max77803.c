...
static void max77803_reg_snapshot(u8 reg, u8 value)
{
	if (reg < MAX77803_MUIC_REG_END)
		muic_reg_snapshot[reg] = value;
}
u8 max77803_restore_last_snapshot(u8 reg)
{
...
/* WA for MUIC RESET */
	if (i2c->addr == I2C_ADDR_MUIC)
		max77803_reg_snapshot(reg, value);
/* WA for MUIC RESET */
...
/* WA for MUIC RESET */
		if (i2c->addr == I2C_ADDR_MUIC)
			max77803_reg_snapshot(reg, new_val);
/* WA for MUIC RESET */
