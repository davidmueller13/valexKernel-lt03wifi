...
struct mif_bus_opp_table mif_bus_opp_list[] = {
	{LV_0, 800000, 1000000, 0},
	{LV_1, 733000,  987500, 0},
	{LV_2, 667000,  962500, 0},
	{LV_3, 533000,  900000, 0},
	{LV_4, 400000,  837500, 0},
	{LV_5, 266000,  825000, 0},
	{LV_6, 200000,  825000, 0},
	{LV_7, 160000,  825000, 0},
	{LV_8, 133000,  825000, 0},
};
...
int exynos5_mif_register_notifier(struct notifier_block *nb)
{
	return srcu_notifier_chain_register
		(&exynos5_mif_transition_notifier_list, nb);
}
...
int exynos5_mif_notify_transition(struct devfreq_info *info, unsigned int state)
{
	...
	return srcu_notifier_call_chain(&exynos5_mif_transition_notifier_list,
								state, info);
}
...
{
	return srcu_notifier_chain_register
		(&exynos5_mif_bpll_transition_notifier_list, nb);
}
...
int exynos5_mif_bpll_transition_notify(struct devfreq_info *info,
					unsigned int state)
{
	BUG_ON(irqs_disabled());

	return srcu_notifier_call_chain
		(&exynos5_mif_bpll_transition_notifier_list, state, info);
}
		...
		unsigned long old_freq, unsigned long target_freq)
{
	...
	if (target_idx <= LV_4) {
		if (!pm_qos_request_active(&exynos5_int_qos))
			pm_qos_add_request(&exynos5_int_qos,
				PM_QOS_DEVICE_THROUGHPUT, 111000);
	} else {
		if (pm_qos_request_active(&exynos5_int_qos))
	...
	if (clk_set_parent(data->mclk_cdrex, data->mx_mspll_ccore)) {
		pr_err("Unable to set mx_mspll_ccore as mclk_cdrex parent\n");
	}
	do {
		cpu_relax();
	...
	if (target_freq <= mif_bus_opp_list[LV_5].clk &&
				!data->changed_timeout) {
		...
		struct devfreq_dev_status *stat)
{
	...
	if (en_profile)
		pr_info("%lu,%lu\n", tmp_nocp_cnt.total_byte_cnt,
					tmp_nocp_cnt.cycle_cnt);
...
static int exynos5420_mif_table(struct busfreq_data_mif *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(mif_bus_opp_list); i++) {
		/* Use default voltage table for now */
		asv_volt = mif_bus_opp_list[i].volt;

		pr_info("MIF %luKhz Voltage is %duV\n", mif_bus_opp_list[i].clk,
								asv_volt);
				...
				char *buf)
{
	...
	for (i = LV_0; i < LV_END; i++)
		len += snprintf(buf + len, write_cnt, "%ld %llu\n",
				mif_bus_opp_list[i].clk,
				(unsigned long long)
					mif_bus_opp_list[i].time_in_state);
...
static DEVICE_ATTR(upthreshold, S_IRUGO | S_IWUSR, show_upthreshold,
						store_upthreshold);

static ssize_t show_target_percentage(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",
...
static DEVICE_ATTR(target_percentage, S_IRUGO | S_IWUSR,
			show_target_percentage, store_target_percentage);

static ssize_t show_proportional(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",
...
static DEVICE_ATTR(proportional, S_IRUGO | S_IWUSR, show_proportional,
						store_proportional);

static ssize_t show_en_profile(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
...
static DEVICE_ATTR(en_profile, S_IRUGO | S_IWUSR, show_en_profile,
						store_en_profile);
...
static struct exynos_devfreq_platdata exynos5420_qos_mif = {
	.default_qos = 160000,
};
...
static int exynos5_devfreq_probe(struct platform_device *pdev)
{
	...
	/* Get clock */
	data->mclk_cdrex = clk_get(dev, "mout_clk_cdrex");
	...
	data->mx_mspll_ccore = clk_get(dev, "mout_mspll_ccr");
	...
	regist_nocp(&mif_noc_list, exynos5_mif_nocp_list,
			ARRAY_SIZE(exynos5_mif_nocp_list));
	...
	if (!pdata)
		pdata = &exynos5420_qos_mif;

	pm_qos_add_request(&exynos5_mif_qos, PM_QOS_BUS_THROUGHPUT,
						pdata->default_qos);
...
static int __init exynos5_devfreq_init(void)
{
	...
	exynos5_devfreq_mif_device.dev.platform_data = &exynos5420_qos_mif;
